...
const PlannerCourseHolder: React.FC<PlannerCourseHolderProps> = ({
  ...
  return (
    <div
      className={`h-20 p-4 flex justify-center items-center w-full border-[#c3a9a9] border-dashed border-2 rounded-xl transition-all duration-200 ease-in-out ${isHover ? "bg-darkblue/20" : ""}`}
    >
      {isHover ? (
        <p>Drop the course!</p>
      ) : (
